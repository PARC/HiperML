#ifndef HML_PAGERANK_SPMV_KERNEL_TEMPLATE_H_INCLUDED_
#define HML_PAGERANK_SPMV_KERNEL_TEMPLATE_H_INCLUDED_
#include "hml_types.h"
...
extern texture<float, 1> texDataMap0;

extern texture<float, 1> texDataMap1;
...
static __inline__ __device__ float texMap0(const int32_t &i) {
  return tex1Dfetch(texDataMap0, i);
}

static __inline__ __device__ float texMap1(const int32_t &i) {
  return tex1Dfetch(texDataMap1, i);
}
...
__global__ void
hmlPagerankSpmvKernelEvenIter0(float           *map1,         /* output map */
                               const uint32_t  *R,
                               const uint32_t  *E,
  ...
  if(rank < maxVertexRank) {
    ...
    if(useTextureMem) {
      ...
      while(minSuccIdx < maxSuccIdx) {
        inputProbSum += texMap0(texE(minSuccIdx)) / texE(minSuccIdx + 1);
        minSuccIdx += 2;
      }
    }
    else {
      //uint32_t       minSuccIdx = R[v];
      //const uint32_t maxSuccIdx = R[v + 1];
      //while (minSuccIdx < maxSuccIdx) {
      //  inputProbSum += texMap0(E[minSuccIdx]) / E[minSuccIdx + 1];
      //  minSuccIdx += 2;
      //}
      const uint32_t  *succ    = &E[R[v]];
      const uint32_t  *succMax = &E[R[v + 1]];
      while(succ < succMax) {
        inputProbSum += texMap0(succ[0]) / succ[1];
        succ += 2;
      }
    }
...
template<bool useTextureMem>
__global__ void
hmlPagerankSpmvKernelOddIter0(float           *map0,         /* output map */
                              const uint32_t  *R,
                              const uint32_t  *E,
                              const uint32_t  *vertexRank,
                              const uint32_t   minVertexRank, /* inclusive */
  ...
  if(rank < maxVertexRank) {
    ...
    if(useTextureMem) {
      ...
      const uint32_t maxSuccIdx = texR(v + 1);
      while(minSuccIdx < maxSuccIdx) {
        inputProbSum += texMap1(texE(minSuccIdx)) / texE(minSuccIdx + 1);
        minSuccIdx += 2;
      }
    }
    else {
      ...
      //const uint32_t maxSuccIdx = R[v + 1];
      //while (minSuccIdx < maxSuccIdx) {
      //  inputProbSum += texMap1(E[minSuccIdx]) / E[minSuccIdx + 1];
      //  minSuccIdx += 2;
      //}
      const uint32_t  *succ    = &E[R[v]];
      const uint32_t  *succMax = &E[R[v + 1]];
      while(succ < succMax) {
        inputProbSum += texMap1(succ[0]) / succ[1];
        succ += 2;
      }
    }
...
template<bool useTextureMem>
__global__ void
hmlPagerankSpmvKernelEvenIter1(float           *map1,         /* output map */
                               const uint32_t  *R,
                               const uint32_t  *E,
                               const uint32_t  *vertexRank,
                               const uint32_t   minVertexRank, /* inclusive */
  ...
  if(rank < maxVertexRank) {
    ...
    if(useTextureMem) {
      uint32_t       minSuccIdx = texR(v);
      const uint32_t maxSuccIdx = texR(v + 1);
      for(minSuccIdx += tid * 2;
          minSuccIdx < maxSuccIdx;
          minSuccIdx += cHmlThreadsPerWarp * 2) {
        psum += texMap0(texE(minSuccIdx)) / texE(minSuccIdx + 1);
      }
    }
    else {
      /*
      const uint32_t *succ    = &E[R[v]];
      const uint32_t *succMax = &E[R[v + 1]];
      for (succ += tid * 2; succ < succMax; succ += cHmlThreadsPerWarp * 2) {
        psum += texMap0(succ[0]) / succ[1];
      }
      */
      const uint32_t minSuccIdx = R[v];
      const uint32_t maxSuccIdx = R[v + 1];
      const uint32_t succs      = maxSuccIdx - minSuccIdx;
      const uint32_t succIdx    = minSuccIdx + tid * 2;
      for(int succ = 0; tid * 2 + succ < succs; succ += cHmlThreadsPerWarp * 2) {
        psum += texMap0(E[succIdx + succ]) / E[succIdx + succ + 1];
      }
    }
  }
...
template<bool useTextureMem>
__global__ void
hmlPagerankSpmvKernelOddIter1(float           *map0,         /* output map */
                              const uint32_t  *R,
                              const uint32_t  *E,
                              const uint32_t  *vertexRank,
                              const uint32_t   minVertexRank, /* inclusive */
  ...
  if(rank < maxVertexRank) {
    ...
    if(useTextureMem) {
      uint32_t       minSuccIdx = texR(v);
      const uint32_t maxSuccIdx = texR(v + 1);
      for(minSuccIdx += tid * 2; minSuccIdx < maxSuccIdx; minSuccIdx += cHmlThreadsPerWarp * 2) {
        psum += texMap1(texE(minSuccIdx)) / texE(minSuccIdx + 1);
      }
    }
    else {
      /*
      ...
      */
      ...
      const uint32_t succIdx    = minSuccIdx + tid * 2;
      for(int succ = 0; tid * 2 + succ < succs; succ += cHmlThreadsPerWarp * 2) {
        psum += texMap1(E[succIdx + succ]) / E[succIdx + succ + 1];
      }
    }
  }
  inputProbSum[tid] = psum;
...
template<bool useTextureMem>
__global__ void
hmlPagerankSpmvKernelEvenIter2(float           *map1,         /* output map */
                               const uint32_t  *R,
                               const uint32_t  *E,
                               const uint32_t  *vertexRank,
                               const uint32_t   minVertexRank, /* inclusive */
  ...
  uint32_t   v;
  float  psum = 0.0;
  __shared__ float  inputProbSum[cHmlPagerankSpmvThreadsPerBlock];
  ...
  if(rank < maxVertexRank) {
    ...
    if(useTextureMem) {
      ...
      for(minSuccIdx += tid * 2;
          minSuccIdx < maxSuccIdx;
          minSuccIdx += cHmlPagerankSpmvThreadsPerBlock * 2) {
        psum += texMap0(texE(minSuccIdx)) / texE(minSuccIdx + 1);
      }
    }
    else {
      ...
      const uint32_t *succ    = &E[R[v]];
      const uint32_t *succMax = &E[R[v + 1]];
      for (succ += tid * 2; succ < succMax; succ += cHmlPagerankSpmvThreadsPerBlock * 2) {
              psum += texMap0(succ[0]) / succ[1];
      }
      */
      const uint32_t minSuccIdx = R[v];
      ...
      const uint32_t succs      = maxSuccIdx - minSuccIdx;
      const uint32_t succIdx    = minSuccIdx + tid * 2;
      for(int succ = 0; tid * 2 + succ < succs; succ += cHmlPagerankSpmvThreadsPerBlock * 2) {
        psum += texMap0(E[succIdx + succ]) / E[succIdx + succ + 1];
      }
    }
  }
...
template<bool useTextureMem>
__global__ void
hmlPagerankSpmvKernelOddIter2(float           *map0,         /* output map */
                              const uint32_t  *R,
                              const uint32_t  *E,
                              const uint32_t  *vertexRank,
                              const uint32_t   minVertexRank, /* inclusive */
...
template<bool useTextureMem>
__global__ void
hmlPagerankSpmvKernelEvenIter3(float           *map1,         /* output map */
                               const uint32_t  *R,
                               const uint32_t  *E,
                               const uint32_t  *vertexRank,
                               const uint32_t   minVertexRank, /* inclusive */
...
template<bool useTextureMem>
__global__ void
hmlPagerankSpmvKernelOddIter3(float           *map0,         /* output map */
                              const uint32_t  *R,
                              const uint32_t  *E,
                              const uint32_t  *vertexRank,
                              const uint32_t   minVertexRank, /* inclusive */
 ...
 *   case 4:
 *     pHmlPagerankSpmvKernelArg[p].block.x = cHmlThreadsPerWarp;
 *     pHmlPagerankSpmvKernelArg[p].block.y = pHmlPagerankSpmvKernelArg[p].block.z = 1;
 *     numBlocks = (numVertices + cHmlThreadsPerWarp - 1) / cHmlThreadsPerWarp;
 *     // gridDimX must >= 8, or CUDA behaves strangely
 *     hmlPagerankSpmvGridDimCalc(&pHmlPagerankSpmvKernelArg[p].grid, numBlocks, 8, cHmlMaxGridDimX);
 *     break;
 */
__global__ void
pagerankEven4(uint32_t  *R,
  ...
  while(doneDeg < totalDeg) {
    ...
    if(tid < maxTid) {
      inputProb[tid] = texMap0(*localR[tid]] * (*(float *)(localR[tid] + 1));
    }
    doneDeg += cHmlThreadsPerWarp;
    //__syncthreads();
    /* PagerankSpmv (possibly partial) reduction */
...
#endif /* HML_PAGERANK_SPMV_KERNEL_TEMPLATE_H_INCLUDED_ */
